...
function VerificarEntradas() { //Cria a função.
    ...
    var nome = document.getElementById("nome").value; //Procura o elemento pelo ID e salva o valor em uma variável.
    var sobrenome = document.getElementById("sobrenome").value; //Procura o elemento pelo ID e salva o valor em uma variável.
    
    if(email == "" || senha == "" || nome == "" || sobrenome == "") { //Verifica se os campos são nulos.
...
function DesabilitarEnviar(tf) { //Cria a função.
    document.getElementById("register").disabled = tf; //Procura o elemento pelo ID e desabilita conforme o valor enviado ao chamar a função.
    if(tf == true){ //Verifica se o valor enviado pra função é "true".
        document.getElementById("register").setAttribute("class", "button disabled"); //Se for verdadeiro procura pelo elemento pelo ID e seta um atributo pro elemento.
    } else {
        document.getElementById("register").setAttribute("class", "button"); //Se for diferente de verdadeiro procura pelo elemento pelo ID e seta um atributo pro elemento.
    }
}
...
function SenhaCaps(event) { //Cria a função.
    ...
    if(tf == true){ //Verifica se o valor enviado pra função é "true".
        ...
        document.getElementById("Warning_Caps").style.opacity = "0"; //Se for verdadeiro diferente de procura pelo elemento pelo ID e seta a opacidade.
    }
}